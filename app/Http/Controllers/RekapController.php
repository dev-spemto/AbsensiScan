    ...
    public function index(Request $request)
    {
        try {
            ...
            */
            ...
            $izinDisetujui = Izin::with('siswa.kelas')
                ->where('status', 'Disetujui');

            if ($request->filled('tanggal')) {
                $izinDisetujui->whereDate('tanggal', $request->tanggal);
            }

            if ($request->filled('bulan')) {
                $izinDisetujui->whereMonth('tanggal', $request->bulan);
            }

            if ($request->filled('kelas')) {
                $izinDisetujui->whereHas('siswa', function ($q) use ($request) {
                    $q->where('kelas_id', $request->kelas);
                });
            }

            if ($request->filled('siswa')) {
                $izinDisetujui->where('siswa_id', $request->siswa);
            }

            $izinDisetujui = $izinDisetujui->get();
            ...
            */
            ...
            return view('rekap.index', [
                ...
                'scanners' => User::where('aktif', true)
                    ->whereIn('role', [
                        'admin',
                        'guru',
                        'ketua_kelas',
